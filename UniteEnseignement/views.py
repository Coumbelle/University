...
def search(request):
	if request.method == "POST":
		...
		print(nom)
		ensei= Enseignant.objects.filter(Q(nom = nom )| Q(prenom =nom))
		text=" "
		filiere=Filiere.objects.filter(id=filiere)
		if not nom:
			text="n\'existe pas"
			if not filiere:
				ens=EnsFiliere.objects.all()
				context ={
					'enseignants':ens,
					'text':text,
				}
				...
				ens= EnsFiliere.objects.get(filiere = filiere)
				context ={
					'enseignants':ens,
					'text':text,
			...
			if not filiere:
				ens= EnsFiliere.objects.get(RespoFiliere=ensei)
				context ={
					'enseignants':ens,
					'text':text,
				}
				...
				enseignants= EnsFiliere.objects.get(Q(RespoFiliere= ensei )| Q(filiere =filiere))
				context ={
...
def listeEns(request):
	enseignants = EnsFiliere.objects.all()
...
########Générer un fichier pdf d'une page html
class GeneratePDF(View):
	def get(self, request, *args, **kwargs):
		datas= Etudiant.objects.all()
		i=1
		context = {
		'datas':datas,
		'i':i
		...
		pdf = render_to_pdf('telecharger.html', context)
